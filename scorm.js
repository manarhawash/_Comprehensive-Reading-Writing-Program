...
var Scorm = (function () {
  ...
  Scorm.prototype.findAPI = function findAPI (win) {
  	...
  	while ((!win.API && !win.API_1484_11) &&
		...
		// EDIT HERE MATAN TO LAUNCHER CHECK 11.09.2024
		if (!win.API_1484_11 && !win.API ) {
			var findAPITries = 0;
			var maxTries = 50;
			var currentWin = win;
			
			while ((!currentWin.API && !currentWin.API_1484_11) &&
				   (currentWin.opener) &&
				   (currentWin.opener.parent) &&
				   (findAPITries <= maxTries)) {
				findAPITries++;
				currentWin = currentWin.opener.parent;
				if(currentWin.API || currentWin.API_1484_11){
					win = currentWin;
					break;
				}
			}
		  ...
		  //  END OF EDIT HERE MATAN TO LAUNCHER CHECK 11.09.2024
